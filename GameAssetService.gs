...
var GAME_ASSET_PROJECT = 'alquimia_do_saber';
var GAME_ASSET_FILES = ['laboratorio_map.svg', 'alquimista_avatar.png'];
var GAME_ASSET_OPTIONAL_FILES = [];

function getGameAssetManifest() {
  return _gameAssetServiceResult_(GameAssetService_getManifest_(GAME_ASSET_FILES, GAME_ASSET_OPTIONAL_FILES, GAME_ASSET_PROJECT));
}

/** Envelope canônico sem perder o manifesto consumido pelo cenário. */
function _gameAssetServiceResult_(manifest) {
  manifest = manifest || {};
  if (manifest.ok === true) return { success: true, data: manifest, error: null };
  return {
    success: false,
    data: manifest,
    error: String(manifest.error || 'Manifesto de assets indisponível.')
  };
}
